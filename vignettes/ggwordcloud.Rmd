...
- `native_speakers`: the number of native speakers (in millions) of thoses languages
- `speaker`: the corresponding total number of speakers (in millions)
The first one `love_words` contains 147 different languages while the second `love_words_small` containes the 34 languages having more than 50 millions speakers.
...
```{r}
for (shape in c("circle", "cardioid", "diamond",
      "square", "triangle-forward", "triangle-upright",
      "pentagon", "star")) {
  set.seed(42)
...
`ggwordcloud` allows to specify a mask within which the words should be placed. More precisely, the _black_ pixels in an image array will be used as a mask:
```{r}
set.seed(42)
ggplot(love_words_small, aes(label = word, size = speakers)) + 
...
We are now ready to make a _lovely_ wordcloud:
...
set.seed(42)
ggplot(love_words_small_l,
  aes(label = word, size = abs(propdelta),
